...
We are trying to find out how much Brownlow can win or lose based on class size and payoff odds.
...
for (i in 1:100){
  money[i] = BrLw(numInClass,odds)
}
hist(money)
```
This means that on average in a class of 35, Brownlow will win around 7.85 per game.
...
```
 ...
 Based on the chart, Brownlow's average payoff should be around 4.50
...
hist(money)
```
In order to make the game fair when the payoff is 2:1, the class size should be 28.
...
Professor Brownlow can change his payoff by changing the class size or the odds. That concludes the results.
